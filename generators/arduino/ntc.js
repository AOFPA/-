...
Blockly.Arduino.ntc_config = function (block) {
    ...
    */
    ...
    Blockly.Arduino.definitions_["define_ntc_config_" + PIN_NTC] =
        ...
        '#define ' + PIN_NTC + ' ' + pin + '\n' +
        '#define ' + R0 + ' ' + R + ' //Ω\n' +
        'float ' + Read + ',' + VR + ',' + RT + ',' + Temp + ',' + ln + ',' + cel + ',' + kel + ',' + fah + ';\n';

    //ส่วน void setup() รวม
    Blockly.Arduino.setups_["setup_ntc_config"] =
        'Temperature0 = 25+273.15;  //Temperature Temperature0 from datasheet, conversion from Celsius to kelvin\n'
        ;
    ...
    code += `${ln} = log(${RT} / ${RT0});\n`
    code += `${Temp} = (1 / ((${ln} / B) + (1 / Temperature0))); //Temperature from sensor\n`
